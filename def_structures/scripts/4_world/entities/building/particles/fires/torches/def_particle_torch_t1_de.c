class DEF_PARTICLE_TORCH_T1_DE extends House
{
    Particle        m_Ptc;          //FIRE
    ...
    void DEF_PARTICLE_TORCH_T1_DE()
    {
        if ( !GetGame().IsDedicatedServer() )
        {
            if (!m_Ptc)
            {
                m_Ptc = ParticleManager.GetInstance().PlayOnObject(ParticleList.TORCH_T1, this, Vector(0.0, 0.0, 0.0));
            }
            if (!m_BoundPtc)
            {
