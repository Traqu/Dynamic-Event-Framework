class DEF_PARTICLE_TORCH_T3_DE extends House
{
    Particle        m_Ptc;          //FIRE
    ...
    void DEF_PARTICLE_TORCH_T3_DE()
    {
        if ( !GetGame().IsDedicatedServer() )
        {
            if (!m_Ptc)
            {
                m_Ptc = ParticleManager.GetInstance().PlayOnObject(ParticleList.TORCH_T3, this, Vector(0.0, 0.0, 0.0));
            }
            if (!m_BoundPtc)
            {
